...
async function load(force = false) {
  ...
  const res = await withRetry(() => sheets.spreadsheets.values.batchGet({ spreadsheetId: SHEET_ID, ranges }), 'load');
  const out = {};
...
/**
 * Rewrite many tabs at once using the FEWEST API calls: one batchClear + one
 * batchUpdate regardless of how many tabs change. This is the key fix for the
 * "Write requests per minute per user" (60/min) quota — a multi-tab save that
 * used to cost ~2 calls per tab now costs 2 calls total.
...
async function batchRewrite(tablesObj) {
  ...
  const clearRanges = names.map(n => `'${n}'!A2:BZ100000`);
  await withRetry(() => sheets.spreadsheets.values.batchClear({ spreadsheetId: SHEET_ID, requestBody: { ranges: clearRanges } }), 'batchClear');
  const data = names
    .filter(n => (tablesObj[n] || []).length)
    .map(n => ({
      range: `'${n}'!A2`,
      values: tablesObj[n].map(o => (_headerCache[n] || []).map(h => _toCell(o[h]))),
    }));
  if (data.length) {
...
module.exports = {
  ...
  appendRows, rewriteTable, batchRewrite, logChange,
};
